...
2. 回波时间（Echo Time，TE）：从发射 [[MRI_基础#^7c6c38|射频脉冲]] 到接受回波信号之间的时间 ^c44b0e
...
下图中，全脑的扫描顺序为 1-3-5-7-2-4-6-8，表示为先扫描奇数层后偶数层，其中 [[fMRI_预处理#^d0b289|TR]] =2s。
...
应用傅里叶变换后的高通滤波或时域平均方法以减少 [[fMRI_预处理#^15f66c|低频漂移]]，或称为去趋势（Detrending）
...
目的：以减少个体间的 [[fMRI_预处理#^e40217|解剖变异]] ，从而成功地进行有意义的群体分析。
